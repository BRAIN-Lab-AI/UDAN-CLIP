...
import sys
import sys
sys.path.append('/hdd/afrahshaahid/models/CLIP-UIE_exp5/model/sr3_modules/CLIP')
from CLIP.clip import load
...
class L_clip_from_feature(nn.Module):
	def __init__(self):
		...
		self.attention = nn.Sequential(
			nn.Conv2d(2048, 256, kernel_size=3, padding=1),  # <-- change 1024 to 2048
			nn.ReLU(),
			nn.Conv2d(256, 1, kernel_size=1),
			nn.Sigmoid()
		)
		# Load ResNet CLIP model for spatial features
		self.res_model, _ = load("RN101", device=device, download_root="./clip_model/")
	...
	def forward(self, x, text_features):
		...
		for i in range(batch_size):
			img = x[i].unsqueeze(0)
			img = F.interpolate(img, size=224, mode='bilinear')  # Resize to 224x224
			img = clip_normalizer(img)
			
			# Extract spatial features from ResNet's layer4
			with torch.no_grad():
				# Use layer4 output as spatial features
				x_stem = self.res_model.visual.relu1(self.res_model.visual.bn1(self.res_model.visual.conv1(img.half())))
				x_stem = self.res_model.visual.relu2(self.res_model.visual.bn2(self.res_model.visual.conv2(x_stem)))
				x_stem = self.res_model.visual.relu3(self.res_model.visual.bn3(self.res_model.visual.conv3(x_stem)))
				x_stem = self.res_model.visual.avgpool(x_stem)
				x1 = self.res_model.visual.layer1(x_stem)
				x2 = self.res_model.visual.layer2(x1)
				x3 = self.res_model.visual.layer3(x2)
				spatial_features = self.res_model.visual.layer4(x3)  # [1, 1024, 7, 7] or similar
				spatial_features = spatial_features.float()  # Convert to float32 for attention layer
			
			# Generate attention mask
			attention_mask = self.attention(spatial_features)  # [1, 1, H, W]
			
			# Apply attention and pool
			attended_features = spatial_features * attention_mask
			attended_features = attended_features.to(self.res_model.visual.attnpool.q_proj.weight.dtype)
			image_features = self.res_model.visual.attnpool(attended_features)  # [1, 512] or [1, output_dim]
...
def load_learned_prompt(path='/hdd/afrahshaahid/models/CLIP-UIE_exp5/weights/iter_1000.pth', length_prompt=16):
